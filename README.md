...
> **同樣缺 20% 的資料，缺法不同，模型受的傷差 37 倍。**
> 所以「缺了多少」不足以描述資料品質——還得知道「怎麼缺的」。
> 這個專案量化缺失結構造成的傷害，並給出一組可操作的資料品質門檻。
...
在高傷害情境（逐筆缺失率 0–40%、六成為 MNAR，整體 AUROC 掉到 0.844）下，
用「最長缺口」或「缺失比例」排序後只回答品質最好的前 40%，AUROC 回到 0.89 附近，
**明顯高於隨機排序的對照組**（0.8964 對 0.8425）。
以下數值皆為 3 個注入 seed 的平均，完整曲線見 [`results/m5c_gate_curves.csv`](results/m5c_gate_curves.csv)。

> 但要誠實兩件事：(a) 覆蓋 40% 時的 0.8964 高於乾淨基線 0.8894，那是**選樣效應**，
...
而我讀到的補值評估多半以隨機遮蔽為主——那剛好落在傷害最小的那一端。
...
6. **本研究沒有做系統性的文獻回顧。** 文中提到「多半以隨機遮蔽評估」時，
   指的是作者自己讀到的範圍，不是對整個文獻的統計。已知也有研究在處理結構化遮罩。
